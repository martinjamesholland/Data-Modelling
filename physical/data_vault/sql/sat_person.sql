...
CREATE TABLE IF NOT EXISTS data_vault.sat_person (
    ...
    -- Constraints
    CONSTRAINT pk_sat_person PRIMARY KEY (sat_person_key, load_date) NOT ENFORCED
)
...
-- Create a view for the current state of the person satellite
CREATE OR REPLACE VIEW data_vault.sat_person_current AS
SELECT * 
FROM data_vault.sat_person
WHERE is_current = TRUE;

-- Create a view for daily batch updates
CREATE OR REPLACE VIEW data_vault.sat_person_daily AS
SELECT * 
FROM data_vault.sat_person
