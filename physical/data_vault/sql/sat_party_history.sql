...
CREATE TABLE IF NOT EXISTS data_vault.sat_party_history (
    ...
    is_current BOOLEAN NOT NULL,              -- Flag for the current version (helpful for filtering)
    ...
    -- Constraints
    CONSTRAINT pk_sat_party_history PRIMARY KEY (sat_party_key, load_date) NOT ENFORCED
)
...
CREATE OR REPLACE VIEW data_vault.sat_party_history_by_day AS
SELECT 
    hub_party_key,
...
CREATE OR REPLACE VIEW data_vault.sat_party_history_realtime AS
SELECT * 
FROM data_vault.sat_party_history
